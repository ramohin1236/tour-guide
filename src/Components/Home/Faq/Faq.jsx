...
const Faq = () => {
  ...
  return (
    <section className="  mx-auto py-32  relative" id="faq">
        <p className="text-center text-base text-[#202020]" data-aos="fade-up" data-aos-duration="3000">
        Don't miss out on the fun! We will help you answer some questions that make you ponder.
      </p>
      <h2 className="text-center mt-4 mb-8 text-3xl md:text-6xl font-semibold text-[#202020]" data-aos="fade-up" data-aos-duration="3000">
        FREQUENTLY A<span className="text-red-600">SK QUESTIONS</span>
      </h2>
    <div className="max-w-5xl border-2 mx-auto px-4">
      <img  src="/rsz_grad2_black.png" alt="" className="absolute top-0 left-0  w-full pointer-events-none " />
      
      <ul className="mt-8">
        {faqs.map((faq, index) => (
          <li key={index} className="border-b last:border-b-0">
            <div
              className="bg-white text-black p-4 flex justify-between items-center cursor-pointer"
            ...
            <div
              className={`bg-gray-300 overflow-hidden transition-all duration-500 ${activeIndex === index ? 'max-h-screen p-4' : 'max-h-0 p-0'}`}
            >
              <p>{faq.answer}</p>
            </div>
          </li>
